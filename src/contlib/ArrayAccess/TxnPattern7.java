...
public class TxnPattern7 extends ArrayAccessTxnMultipleFuture {
	VBox<Integer>[] array;
	private int value;
	...
	public TxnPattern7(int max_num_of_core, int prefix_disjoint_read, int hot_spot_in_the_array,int num_of_hot_spots_read_and_write, String high_contention, int spin){
	...
	private class AsyncOperation1 implements Callable<Integer>{
		...
		public Integer call() throws InterruptedException {
			...
			for(int i=0 ; i< 2; i++){
				int theadNum = (int) (Thread.currentThread().getId()%56);
				int partition = array_length /56;
				int read_index = (int) (Math.random()*partition + theadNum*partition);
				value +=  array[read_index].get();

				int write_index = (int) (Math.random()*partition + theadNum*partition);
